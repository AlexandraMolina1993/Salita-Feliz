-- ==============================================================================
-- MIGRACIÓN DE ARQUITECTURA DE INVENTARIO CLÍNICO, VISTA EN TIEMPO REAL Y RPC
...
-- 2. Asegurar columnas en tablas base
...
-- Asegurar columnas y relajar restricciones en vaccination_records
...
-- 3. Tabla del Libro Mayor Inmutable de Movimientos de Stock (stock_movements)
...
COMMENT ON TABLE stock_movements IS 'Ledger inmutable de auditoría para todos los movimientos de stock de vacunas (consumos, reposiciones, ajustes e incidentes)';

-- 4. Tabla de Auditoría del Agente de IA (ai_notifications_log)
...
-- 5. Vista Dinámica en Tiempo Real de Balance de Inventario (v_vaccines_stock)
...
-- 6. Función Transaccional Atómica RPC (process_appointment_completion)
...
BEGIN
    ...
    -- 6. Insertar registro histórico en vaccination_records
...
-- 7. Permisos para que la API autenticada y anónima puedan consultar la vista y ejecutar la RPC
...
GRANT ALL ON ai_notifications_log TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION process_appointment_completion TO anon, authenticated, service_role;

-- 8. Notificar a PostgREST para recargar el Schema Cache inmediatamente
