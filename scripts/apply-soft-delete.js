...
const migrationFile = path.join(__dirname, '07-soft-delete-appointments.sql');
...
async function run() {
  ...
  try {
    ...
    const res = await client.query("SELECT column_name, data_type, is_nullable FROM information_schema.columns WHERE table_name = 'appointments' AND column_name = 'deleted_at';");
